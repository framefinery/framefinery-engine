...
ENCODE_MATRIX_AV2_GOP_FLAG := --av2-gop "$(ENCODE_MATRIX_AV2_GOP)"
ENCODE_MATRIX_VVC_GOP_FLAG := --vvc-gop "$(ENCODE_MATRIX_VVC_GOP)"
...
ENCODE_MATRIX_FAIL_ON_REGRESS_FLAG := $(if $(filter 1 true yes,$(ENCODE_MATRIX_FAIL_ON_REGRESS)),--fail-on-regress,)
...
help:
	@printf '%s\n' \
		...
		'  make api-docs         Alias for make doc; writes target/doc/framefinery/index.html' \
		'  make api-docs-strict  Fail if exported API is missing /// documentation' \
		...
		'                         Set ENCODE_MATRIX_BASELINE=path/to/run.json to score tradeoffs' \
		'                         Set ENCODE_MATRIX_FAIL_ON_REGRESS=1 to fail scored regressions' \
...
regression: build
	$(PYTHON) scripts/run_validation_set.py --ff "$(abspath $(BUILD_BINARY))" --codec av2 smoke --set-dir "$(VALIDATION_SET_DIR)" --vector-dir "$(VALIDATION_OUT_DIR)" --encoded-dir "$(VALIDATION_ENCODED_DIR)" --log-dir "$(VALIDATION_LOG_DIR)" --reference-mode "$(VALIDATION_REFERENCE_MODE)" --setting "gop=0" --stop-on-fail
