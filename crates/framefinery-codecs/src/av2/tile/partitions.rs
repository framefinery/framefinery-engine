...
fn choose_8x8_leaf_partition(
    ...
    // splits. FrameFinery's current AV2 MVP fixes the coding leaf to 8x8; any
...
fn partition_trace_enabled() -> bool {
    std::env::var_os("FRAMEFINERY_AV2_PARTITION_TRACE").is_some_and(|value| value != "0")
}
...
fn trace_partition_symbol(
    ...
    if partition_trace_enabled() {
        eprintln!(
            "FRAMEFINERY_AV2_PARTITION_TRACE {} mi=({},{}) block={}x{} bsize_map={} rect_map={} ctx={} symbol={}",
            syntax,
            decision.row,
            decision.col,
...
fn write_intrabc_copy(
    ...
    // differential BV with av2_encode_dv()/ndvc contexts. FrameFinery uses
...
fn write_intrabc_explicit_dv(writer: &mut Av2EntropyWriter, dv: Av2IntrabcExplicitDv) {
    ...
    // intrabc_bv_precision symbol is present. FrameFinery stores IBC vectors
