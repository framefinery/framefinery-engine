...
#[test]
fn vvc_luma_mrl_angular_prediction_uses_vtm_dct_interpolation() {
    let geometry = VvcVideoGeometry {
        width: 16,
        height: 24,
    };
    ...
    let mut luma = vec![0; geometry.width * geometry.height];
    let mut availability = vec![false; geometry.width * geometry.height];
    let mut set_available = |x: usize, y: usize, sample: VvcSample| {
        luma[y * geometry.width + x] = sample;
        availability[y * geometry.width + x] = true;
    };

    for (offset, sample) in [46, 53, 53, 203, 203, 211, 203, 207, 203, 212, 203]
        .iter()
        .copied()
        .enumerate()
    {
        set_available(5 + offset, 13, sample);
    }
    for (offset, sample) in [46, 46, 46, 211, 210, 204, 211, 211, 203, 207, 208]
        .iter()
        .copied()
        .enumerate()
    {
        set_available(5, 13 + offset, sample);
    }
    ...
    prediction::predict_vvc_luma_intra_block_into_with_mrl_and_availability(
        &mut prediction,
        &mut scratch,
        VvcIntraPredictionMode::Angular(35),
        &luma,
        geometry,
        node,
        SampleBitDepth::new(8).expect("valid bit depth"),
        2,
        Some(prediction::VvcPlaneAvailability::new(
            &availability,
            geometry.width,
        )),
    );
    ...
    assert_eq!(
        prediction,
        vec![
            48, 49, 91, 214, 205, 210, 204, 206, 46, 49, 44, 109, 217, 207, 209, 204, 36, 46, 49,
            44, 121, 212, 207, 208, 113, 36, 46, 50, 44, 142, 212, 208, 226, 100, 38, 45, 51, 39,
            154, 209, 210, 221, 82, 41, 46, 51, 44, 170, 211, 210, 219, 72, 41, 46, 52, 46, 211,
            211, 210, 216, 56, 46, 46, 53,
        ]
    );
}
...
fn vvc_transform_skip_reconstruction_uses_encoded_chroma_coefficients() {
    ...
    let residuals_8x8: Vec<i16> = (0..64)
        .map(|index| ((index as i16 * 11) % 31) - 15)
        .collect();
    let (ac_levels_8x8, has_ac) =
        quant::transform_skip_chroma_ac_levels_and_flag(&residuals_8x8, 8);
    assert!(has_ac);
    quant::reconstruct_vvc_chroma_transform_skip_residuals_into(
        &mut reconstructed,
        residuals_8x8[0],
