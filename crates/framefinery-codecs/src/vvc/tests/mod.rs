...
#[test]
fn vvc_lossy_chroma_qp_matches_signaled_sps_mapping_table() {
    for (slice_qp, expected_chroma_qp) in [
        (0, 0),
        (1, 1),
        (17, 17),
        (24, 25),
        ...
        assert_eq!(
            vvc_lossy_chroma_qp_for_slice_qp(slice_qp),
            expected_chroma_qp
        );
    }
}
...
#[test]
fn vvc_predictive_exact_inter_selector_accepts_chroma_safe_444_leaves() {
    let geometry = VvcVideoGeometry {
        width: 16,
        height: 8,
    };
    ...
    let mut previous_luma = vec![0; plane_len];
    let mut previous_cb = vec![0; plane_len];
    let mut previous_cr = vec![0; plane_len];
    let mut current_luma = vec![0; plane_len];
    let mut current_cb = vec![0; plane_len];
    let mut current_cr = vec![0; plane_len];
    for y in 0..geometry.height {
        for x in 0..geometry.width {
            let idx = y * geometry.width + x;
            previous_luma[idx] = ((x * 17 + y * 11) & 0xff) as VvcSample;
            previous_cb[idx] = ((x * 23 + y * 7 + 19) & 0xff) as VvcSample;
            previous_cr[idx] = ((x * 29 + y * 5 + 31) & 0xff) as VvcSample;
        }
        for x in 0..8 {
            let left = y * geometry.width + x;
            let right = y * geometry.width + 8 + x;
            current_luma[left] = previous_luma[right];
            current_luma[right] = previous_luma[left];
            current_cb[left] = previous_cb[right];
            current_cb[right] = previous_cb[left];
            current_cr[left] = previous_cr[right];
            current_cr[right] = previous_cr[left];
        }
    }
    let previous = VvcSampledFrame {
        geometry,
    ...
    .expect("4:4:4 exact chroma motion should allow explicit inter decisions");
...
fn vvc_luma_mrl_candidates_are_gated_by_ctu_line_and_mode() {
    ...
    let lossy_context = VvcResidualModeDecisionContext::new(format, VvcResidualCodingMode::Lossy);
    let lossless_context =
    ...
    below_top_node.y = 8;
    let mut below_top_4x4_node = VvcCodingTreeNode::root(4, 4, VvcTreeType::DualTreeLuma);
    below_top_4x4_node.y = 8;
...
fn vvc_lossy_predictive_mixed_frame_uses_rd_gated_ctu_skip() {
    ...
    assert_eq!(
        predictive_nals
            ...
            .count(),
        1,
        "the mixed lossy predictive frame should be encoded as one trailing P slice"
    );
    assert_eq!(
        predictive_nals
            .iter()
            ...
            .count(),
        0,
        "mixed lossy predictive frames should carry slice state in the single trailing slice"
    );
}

#[test]
fn vvc_lossy_predictive_mixed_yuv422_frame_uses_ctu_slice_fallback() {
    let geometry = VvcVideoGeometry {
        width: 128,
        height: 64,
    };
    let input = yuv422p8_two_frame_right_half_repeated(geometry.width, geometry.height);
    let predictive = vvc_yuv_encode_artifacts_from_input_with_options(
        &input,
        VvcEncodeParams { frames: 2 },
        geometry,
        PixelFormat::yuv422(8).expect("valid 4:2:2 format"),
        VvcEncodeOptions {
            qp: Some(19),
            gop: crate::settings::GopMode::Infinite,
            ..VvcEncodeOptions::default()
    ...
    .expect("predictive 4:2:2 VVC encode should succeed");
...
fn yuv422p8_two_frame_right_half_repeated(width: usize, height: usize) -> Vec<u8> {
    assert_eq!(width, 128);
    assert_eq!(height, 64);
    let format = PixelFormat::yuv422(8).expect("valid 4:2:2 format");
    let mut out = Vec::with_capacity(Picture::expected_len(width, height, format) * 2);
    append_yuv422p8_split_luma_frame(&mut out, width, height, 72, 72, 128, 192);
    append_yuv422p8_split_luma_frame(&mut out, width, height, 96, 72, 128, 192);
    out
}
...
fn append_yuv422p8_split_luma_frame(
    out: &mut Vec<u8>,
    width: usize,
    height: usize,
    left_y: u8,
