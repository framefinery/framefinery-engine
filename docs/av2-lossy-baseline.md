...
FrameFinery Engine QP24 encode speed improves from 2.30 fps to 3.29 fps.
...
```text
cargo test -p framefinery-codecs av2_local_ibc --all-features
cargo test -p framefinery-codecs --all-features
...
already supported by FrameFinery Engine.
...
but the 50-frame comparison remains dominated by FrameFinery Engine's missing lossy
...
manual six-row QP24 FrameFinery Engine refresh under verification/generated/lossy_quality
```
...
mode and TXB choices with `FRAMEFINERY_AV2_LOSSY_STATS`, then keeping only the
...
quality signal, but also that the remaining gap is structural: FrameFinery Engine is
...
bitrate is not: FrameFinery Engine spends roughly 2.9x the first-frame bytes to reach
...
| AVM inspect on raw FrameFinery Engine OBU | n/a | n/a | Rejected as a measurement source: inspect segfaulted on the raw OBU stream |
...
and IntraBC on the screen-content sample. FrameFinery Engine cannot match that pattern
...
cargo fmt --check
cargo check -p framefinery-codecs --all-features
cargo test -p framefinery-codecs --all-features av2_regular_qp -- --nocapture
...
cargo check -p framefinery-codecs --all-features
cargo test -p framefinery-codecs --all-features av2_regular_qp -- --nocapture
cargo test -p framefinery-codecs --all-features av2_lossy -- --nocapture
cargo test -p framefinery-codecs --all-features av2_lossless -- --nocapture
make build
...
search follows the libaom-style shape already used by FrameFinery Engine's regular-Q
...
2,408,342 FrameFinery Engine bytes versus 1,297,001 `aomenc` bytes, or 1.86x. The
50-frame total is 55,376,826 FrameFinery Engine bytes versus 3,028,382 `aomenc` bytes,
...
manual six-row first-frame QP24 predictive encode with FRAMEFINERY_AV2_SB_BITS and FRAMEFINERY_AV2_LOSSY_STATS
...
raster last-position value, while FrameFinery Engine was using the scan position. The
...
```text
cargo fmt --all
cargo check -p framefinery-codecs --features av2
...
cargo test -p framefinery-codecs --features av2
make build AV2_SB_BITS=1 AV2_LOSSY_STATS=1
manual three-row first-frame FRAMEFINERY_AV2_SB_BITS + FRAMEFINERY_AV2_LOSSY_STATS trace
make compare-compression CODEC=av2 COMPRESSION_SET=local-aomctc-b2-scc-1080p-lossless-50f COMPRESSION_REFERENCE_BACKEND=ffmpeg-libaom COMPRESSION_REFERENCE_PRESET=realtime-screen COMPRESSION_SETTINGS=predictive COMPRESSION_QP=24 COMPRESSION_DIRECT_SOURCE_FILES=1
