...
FrameFinery Engine raw input metadata uses explicit pixel-format names. This document
...
|---|---|---|---|---|
| `yuv420p8`, `yuv420p10le` | native lossy/lossless | native lossy/lossless | exact for both codecs | Shared raw metadata requires even width and height. AV2 accepts legal visible dimensions and pads internally to its current 8-pixel coded canvas. VVC currently requires even width and height and pads the coded canvas to 8-pixel granularity. |
| `yuv422p8`, `yuv422p10le` | native lossy/lossless | native lossy/lossless | exact for both codecs | Shared raw metadata requires even width. AV2 accepts legal visible dimensions and pads internally to its current 8-pixel coded canvas. VVC currently requires even width and height. |
| `yuv444p8`, `yuv444p10le`, `gbrp8` | native lossy/lossless | native lossy/lossless | exact for both codecs | AV2 accepts non-zero visible dimensions and pads internally to its current 8-pixel coded canvas. VVC currently requires even width and height. |
