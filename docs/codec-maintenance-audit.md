...
| --- | ---: | --- |
| VVC CABAC CTU generation | approximately 2,350 lines after removing disabled leaf-skip syntax and merging the duplicated luma/chroma walkers; the partition parent is 1,185 lines with focused 174-line geometry, 253-line luma traversal, and 164-line chroma traversal siblings | luma and chroma child selection each have one event traversal with neighbour contexts retained in CABAC adapters; shared geometry has explicit ownership, and remaining work should separate other syntax ownership without recreating traversal paths |
