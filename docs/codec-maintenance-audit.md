...
| --- | ---: | --- |
| VVC CABAC CTU generation | approximately 2,350 lines after removing disabled leaf-skip syntax and merging the duplicated luma/chroma walkers; the partition parent is 750 lines with focused 174-line geometry, 213-line luma split-context, 223-line chroma split-policy, 253-line luma traversal, and 164-line chroma traversal siblings | luma and chroma child selection each have one event traversal; shared geometry, luma context derivation, and chroma legality policy have explicit ownership without duplicating syntax paths; remaining work should separate CTU entry-point and operation-generation ownership without recreating traversal paths |
