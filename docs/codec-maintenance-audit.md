...
| --- | ---: | --- |
| VVC CABAC CTU generation | approximately 2,350 lines after removing disabled leaf-skip syntax and merging the duplicated luma/chroma walkers; the operation-generation parent is 590 lines with focused 78-line parameters, 174-line geometry, 83-line transform planning, 213-line luma split-context, 223-line chroma split-policy, 253-line luma traversal, and 164-line chroma traversal siblings | luma and chroma child selection each have one event traversal; transform planning consumes those shared events, and parameters/geometry/context/legality concerns have explicit ownership without duplicating syntax paths; remaining review should focus on operation-generation cohesion rather than recreating traversal paths |
