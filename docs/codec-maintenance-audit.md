...
| --- | ---: | --- |
| VVC CABAC CTU generation | approximately 2,350 lines after removing disabled leaf-skip syntax and merging the duplicated luma/chroma walkers; the partition parent is 973 lines with focused 174-line geometry, 213-line luma split-context, 253-line luma traversal, and 164-line chroma traversal siblings | luma and chroma child selection each have one event traversal, shared geometry has explicit ownership, and luma neighbour/context derivation is isolated without duplicating intra/inter syntax paths; remaining work should separate other syntax ownership without recreating traversal paths |
