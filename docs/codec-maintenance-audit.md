...
| --- | ---: | --- |
| VVC CABAC CTU generation | approximately 2,434 lines after removing disabled leaf-skip syntax and merging the duplicated luma walkers; the partition parent is 1,480 lines with a focused 253-line traversal sibling | luma child selection now has one event traversal with neighbour contexts retained in its CABAC adapter; remaining work should separate other syntax ownership without recreating traversal paths |
