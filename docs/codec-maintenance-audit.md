...
- AV2 transform-block skip writers now resolve each legal context to one
  ...
  every valid Y, U, and V context before and after the refactor. The descriptor
  table, family wrappers, shared emitter, and contract now live together in a
  438-line included sibling; the 240-line parent retains EOB, DC, Rice, and
  entropy-context helpers in the original module scope.
...
- AV2 forward and inverse DCT kernels now live in focused 239- and 278-line
...
- AV2 inverse DCT entry points now share one bit-depth clipping-range owner,
  one full-transform input clamp, and one const-generic two-stage DC-only
  reconstruction kernel. Geometry wrappers retain their exact DCT kernel,
...
- AV2 coefficient mode scoring and entropy emission now share one focused
  107-line high-range policy and writer sibling. It owns the luma-transform,
  ...
  against the 228-line rate proxy across LF and non-LF cases. The specialized
  luma and chroma DC-only tails now also select their existing field names and
  call the shared LF policy with a fresh context, rather than deriving the
...
- AV2 coefficient-context neighbour lookup for 4x4, 8x8, and 4x8 transforms
  ...
  have one implementation. Chroma EOB bucketing, LF/non-LF neighbour
  aggregation, U/V plane offsets, and base-range contexts also share one
  formula through trait defaults; each transform-size policy now retains only
  its geometry sampler. A focused 144-line test independently and exhaustively
  covers every coefficient position, edge delta, plane, scan index, and EOB
  state for all three geometries.
- AV2 luma LF/non-LF lower-level and base-range contexts now share one
  neighbour-magnitude implementation. The frequency gate supplies only the
  legal clamp, bucket, and context-offset policy at the context-selection
  boundary. The coefficient-context contract is now 274 lines and also covers
  every luma position, scan/EOB state, both frequency policies, sparse
  neighbours, mixed levels, and the 127-level clamp boundary.
- AV2 identity-transform upper-level, base-range, and sign contexts now share
  one 4x4 neighbour-geometry record. Upper/base magnitude contexts use the
  same limited left/above sampler, while sign derivation alone consumes the
...
- VVC luma transform-node collection and CABAC operation generation now share
  ...
  1,732 lines.
- That unified VVC luma partition event traversal now lives in a focused
  253-line included sibling. The 1,480-line parent retains node geometry,
  chroma traversal, split availability, neighbour-context adaptation, and
  public-in-crate entry points. Both files resolve in the same module scope,
  so the extraction changes neither traversal selection nor syntax ownership.
- VVC chroma transform-node planning and CABAC generation now share one
  164-line QT/BT/boundary event traversal across 4:2:0, 4:2:2, and 4:4:4.
  The common walker owns child selection and the existing leaf-size/BT-
  direction policy; its transform adapter records leaves, while the CABAC
  adapter retains neighbour state, contexts, and leaf syntax. Visible and
  inferred-boundary splits now use the same deepest syntax writers with only
  their signal flags gated. This removes the separate 81-line boundary walker,
  duplicate leaf/direction predicates, and unused leaf-size plumbing. A
  pre-refactor/post-refactor contract verifies exact planned-versus-emitted
  leaf counts over 489 legal unusual-geometry combinations.
- The shared VVC CTU partition vocabulary now lives in a focused 174-line
  included sibling: partition shape, tree/split kinds, node geometry, child
  derivation, boundary depth adjustment, and luma tree-kind selection. The
  remaining partition parent is 1,185 lines and retains policy, neighbour
  contexts, CABAC operation generation, and public-in-crate entry points. This
  is an ownership-only extraction in the original module scope.
- VVC luma partition neighbour storage and split-context input formulas now
  live in a focused 213-line included sibling. The partition parent is 973
  lines and retains operation generation and legality policy; neighbour cell
  updates and the `split_cu_flag`/`split_qt_flag` context contracts remain in
  one module-scope implementation shared by intra and inter traversal.
- VVC chroma subsampled geometry, implicit-boundary split derivation, legal
  QT/BT/TT availability, and BT-direction policy now live together in a
  focused 223-line included sibling. The 750-line partition parent retains
  CTU parameter entry points and CABAC operation generation; both transform
  planning and syntax emission still consume the same chroma policy through
  the unified event walker.
- VVC luma and chroma transform-node collection adapters now live together in
  an 83-line included sibling. They expose the existing allocation-reusing
  entry points and collect only leaf events from the unified component
  walkers. The partition parent is 668 lines and no transform-specific
  traversal or split policy was introduced.
- The VVC CTU partition parameter record and its geometry/tree views now live
  in a focused 78-line included sibling. This keeps the luma/chroma TU metadata
  contract together while leaving the 590-line parent focused on CABAC
  operation representation and generation; field visibility and construction
  remain unchanged.
- VVC intra and inter luma split availability now share one QT/BT/TT
  derivation. The traversal selects a limits record once at entry and carries
  only maximum MTT depth, BT/TT sizes, and the inter QT-parent restriction;
  availability and leaf-at-depth handling consume that record at their deepest
  decisions. This removes the independent intra/inter derivations and wrapper
  helpers from the 542-line operation generator. A contract pins the distinct
  64x64 limits, QT-parent gate, and inter depth-three cutoff.
...
| --- | ---: | --- |
| VVC CABAC CTU generation | approximately 2,300 lines after removing disabled leaf-skip syntax and merging the duplicated luma/chroma walkers and luma mode-specific availability derivations; the operation-generation parent is 542 lines with focused 78-line parameters, 174-line geometry, 83-line transform planning, 213-line luma split-context, 223-line chroma split-policy, 273-line luma traversal, and 164-line chroma traversal siblings | luma and chroma child selection each have one event traversal; intra/inter luma availability shares one limits-driven derivation, transform planning consumes the shared events, and parameters/geometry/context/legality concerns have explicit ownership without duplicating syntax paths; remaining review should focus on operation-generation cohesion rather than recreating traversal paths |
