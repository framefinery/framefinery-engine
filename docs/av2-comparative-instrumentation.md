...
This note records the workflow for improving FrameFinery Engine AV2 bitrate while
holding the current PSNR guardrail. The intent is to compare FrameFinery Engine against
...
- Validate changed AV2 bitstreams with AVM decode and compare AVM raw output
  against FrameFinery Engine internal reconstruction.
...
`AV2_SB_BITS=1` enables `framefinery-codecs/av2-sb-bit-profile` and writes
per-superblock JSONL only when `FRAMEFINERY_AV2_SB_BITS` is set at runtime.
`AV2_LOSSY_STATS=1` enables `framefinery-codecs/av2-lossy-stats` and prints
mode/TXB summaries only when `FRAMEFINERY_AV2_LOSSY_STATS` is set at runtime.
...
```sh
FRAMEFINERY_LIBAOM_SB_BITS=verification/generated/instrumentation/libaom.jsonl
FRAMEFINERY_AVM_SB_BITS=verification/generated/instrumentation/avm.jsonl
```
...
```sh
make reference-setup REFERENCE_CODEC=libaom LIBAOM_SB_BITS=1
FRAMEFINERY_LIBAOM_SB_BITS=verification/generated/instrumentation/scene_libaom_sb.jsonl \
...
```sh
make reference-setup REFERENCE_CODEC=av2 AVM_SB_BITS=1
FRAMEFINERY_AVM_SB_BITS=verification/generated/instrumentation/scene_avm_sb.jsonl \
  make compare-compression CODEC=av2 \
  COMPRESSION_SET=local-aomctc-b2-scc-1080p-lossless-50f \
  COMPRESSION_LIMIT=1 \
...
scripts/summarize_encoder_instrumentation.py \
  --sb-bits scene/framefinery=verification/generated/instrumentation/scene_ff_sb.jsonl \
...
- Is FrameFinery Engine overspending uniformly, or only in specific superblocks?
...
```sh
scripts/summarize_encoder_instrumentation.py \
  --lossy-stats-log scene/framefinery=verification/generated/instrumentation/scene_stats.log
```
...
FrameFinery Engine has JSONL entropy field helpers for AV2 syntax traces. When a local
harness emits those traces, summarize the fields with:

```sh
scripts/summarize_encoder_instrumentation.py \
  --field-trace scene/framefinery=verification/generated/instrumentation/scene_fields.jsonl
```
...
1. Generate first-frame and 50-frame scoreboards for FrameFinery Engine and the AV1
...
3. Compare FrameFinery Engine against libaom and AVM at the same frame count.
