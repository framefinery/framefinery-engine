...
baseline. FrameFinery Engine fps is timed locally by the comparison script.
...
- Previous FrameFinery Engine bytes: 297,043,630.
- Current FrameFinery Engine bytes: 340,110,565.
...
- Added `COMPRESSION_SETTINGS` to `make compare-compression`, forwarding
  repeated FrameFinery Engine `--set` values to the measured encoder command.
...
FrameFinery Engine. Use this checkpoint as the active predictive baseline for the next
