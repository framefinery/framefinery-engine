...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            "per-superblock JSONL from FRAMEFINERY_AV2_SB_BITS, "
            "FRAMEFINERY_LIBAOM_SB_BITS, or FRAMEFINERY_AVM_SB_BITS"
        ),
    )
    parser.add_argument(
        "--field-trace",
        ...
        default=[],
        metavar="[CASE/]SOURCE=PATH",
        help="FrameFinery entropy field JSONL trace with name and bit_count fields",
    )
    parser.add_argument(
        "--lossy-stats-log",
        action="append",
        default=[],
        metavar="[CASE/]SOURCE=PATH",
        help="stderr log containing gated FRAMEFINERY_AV2_LOSSY_STATS lines",
    )
    parser.add_argument(
        "--vvc-stats",
        action="append",
        default=[],
        metavar="[CASE/]SOURCE=PATH",
        help="VVC stage timing JSONL from FRAMEFINERY_VVC_STATS",
    )
    parser.add_argument(
        "--baseline-source",
        default="framefinery",
...
def read_lossy_stats_rows(spec: TraceSpec) -> list[tuple[str, str, str, dict[str, int]]]:
    ...
    with spec.path.open("r", encoding="utf-8", errors="replace") as source:
        for line in source:
            ...
            rows.append((spec.case, spec.source or "framefinery", match.group("label"), values))
    return rows
...
def print_lossy_stats_summary(rows: list[tuple[str, str, str, dict[str, int]]]) -> None:
    ...
    print("## FrameFinery Lossy Mode/TXB Stats")
...
def print_vvc_counter_summary(rows: list[VvcCounterRow], top: int) -> None:
    ...
    selected_prefixes = (
        ...
        "chroma_mode",
        "luma_dc_prediction",
        "luma_planar_prediction",
        "luma_directional_prediction",
        "luma_mrl_prediction",
        "luma_bdpcm_prediction",
        "luma_residual_build",
        "luma_rd_",
        "luma_transform_skip",
        "luma_transformed_quant",
        "luma_residual_recon",
        "luma_fill",
        "chroma_derived_prediction",
        "chroma_explicit_prediction",
        "chroma_cclm_prediction",
        "chroma_bdpcm_prediction",
        "chroma_residual_build",
        "chroma_rd_",
        "chroma_transform_skip",
        "chroma_transformed_quant",
        "chroma_residual_recon",
        "chroma_fill",
...
def choose_baseline_source(sources: dict[str, list[SbRow]], preferred: str) -> str | None:
    ...
    for source in sources:
        ...
        if preferred_lower in lowered or "framefinery" in lowered or lowered == "ff":
            return source
    return None
