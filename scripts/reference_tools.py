...
GLOBAL_DECODER_ENV = "FRAMEFINERY_DECODER"
GLOBAL_ENCODER_ENV = "FRAMEFINERY_ENCODER"
GLOBAL_REFERENCE_DIR_ENV = "FRAMEFINERY_REFERENCE_DIR"
LEGACY_GLOBAL_REFERENCE_DIR_ENV = "FRAMEFINERY_REF_DIR"
...
def main() -> int:
    ...
    decode_parser = subparsers.add_parser("decode", help="decode a FrameFinery bitstream")
...
def apply_reference_patches(manifest: ReferenceManifest, root: Path) -> None:
    patch = None
    if manifest.codec == "libaom" and truthy_env("FRAMEFINERY_LIBAOM_SB_BITS_BUILD"):
        patch = REPO_ROOT / "tools" / "libaom-sb-bits.patch"
    elif manifest.codec == "av2" and truthy_env("FRAMEFINERY_AVM_SB_BITS_BUILD"):
...
def build_cmake_reference(manifest: ReferenceManifest, root: Path) -> None:
    ...
    if jobs := os.environ.get("FRAMEFINERY_BUILD_JOBS"):
...
def cmake_args(manifest: ReferenceManifest) -> list[str]:
    ...
    if manifest.codec == "libaom" and truthy_env("FRAMEFINERY_LIBAOM_SB_BITS_BUILD"):
        args.append("-DCMAKE_C_FLAGS=-DFRAMEFINERY_LIBAOM_SB_BITS=1")
    if manifest.codec == "av2" and truthy_env("FRAMEFINERY_AVM_SB_BITS_BUILD"):
        args.append("-DCMAKE_C_FLAGS=-DFRAMEFINERY_AVM_SB_BITS=1")
    return args
