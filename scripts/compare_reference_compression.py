#!/usr/bin/env python3
"""Compare FrameFinery bitstream sizes against a reference encoder."""
...
class ComparisonResult:
    vector_name: str
    framefinery_output: Path
    reference_output: Path
    framefinery_bytes: int
    reference_bytes: int
    ratio: float
    framefinery_seconds: float
    framefinery_fps: float
    frame_count: int
    lossless: bool
    qp: int | None
...
def main() -> int:
    ...
    for index, (vector, vector_path) in enumerate(cases, start=1):
        ...
        print(
            ...
            "FrameFinery={ff} byte(s), reference={ref} byte(s), "
            "ratio={ratio:.3f}x, encode={fps} fps".format(
                ...
                ff=result.framefinery_bytes,
                ref=result.reference_bytes,
                ratio=result.ratio,
                fps=format_fps(result.framefinery_fps),
            ),
            flush=True,
        )
    ...
    print()
    print(
        "FrameFinery media compression comparison: "
        f"{args.set} ({args.codec}, baseline={args.reference_backend})"
    )
    print(
        "| # | vector | mode | reference | FrameFinery bytes | reference bytes | "
    ...
    total_ref = 0
    total_frames = 0
    total_framefinery_seconds = 0.0
    for index, result in enumerate(results, start=1):
        total_ff += result.framefinery_bytes
        total_ref += result.reference_bytes
        total_frames += result.frame_count
        total_framefinery_seconds += result.framefinery_seconds
        delta = result.framefinery_bytes - result.reference_bytes
        mode = result_mode(result)
        cache = "cached" if result.reference_cached else "fresh"
        print(
            f"| {index} | {result.vector_name} | {mode} | {cache} | {result.framefinery_bytes} | "
            f"{result.reference_bytes} | {result.ratio:.3f}x | {delta:+d} | "
            f"{format_fps(result.framefinery_fps)} | "
            f"{relpath(result.log)} |"
        )
    if results:
        total_ratio = total_ff / total_ref if total_ref else float("inf")
        total_fps = (
            total_frames / total_framefinery_seconds
            if total_framefinery_seconds > 0.0
            else float("inf")
        )
        print(
    ...
    print()
    print(
        "NOTE: FrameFinery encode FPS is timed locally; reference encoders are only size baselines."
    )
    return 0
...
def run_case(
    ...
    framefinery_output, reference_output, log = case_paths(Path(vector.filename).stem, args)
    if framefinery_output.exists():
        framefinery_output.unlink()
    lossless = effective_framefinery_lossless(vector, args)
    ...
    if vector.fps is not None:
        framefinery_cmd.extend(["--fps", vector.fps])
    framefinery_cmd.extend(
        [
            "--encode",
            f"{args.codec}:{framefinery_output}",
        ]
    )
    if lossless:
    ...
    framefinery_result, framefinery_seconds = run_logged_timed(framefinery_cmd)
    framefinery_fps = (
        vector.frames / framefinery_seconds if framefinery_seconds > 0.0 else float("inf")
    )
    ...
    log.write_text(
        f"$ {shlex.join(framefinery_cmd)}\n\n{framefinery_result.stdout}\n"
        f"FrameFinery timing: elapsed_seconds={framefinery_seconds:.6f} "
        f"encode_fps={format_fps(framefinery_fps)}\n\n"
        f"{reference_log_header}\n\n{reference_output_text}"
    )

    if framefinery_result.returncode != 0:
        raise SystemExit(f"FrameFinery encode failed for {vector.filename}; see {relpath(log)}")
    ...
    require_non_empty(framefinery_output, "FrameFinery", vector.filename, log)
    if reference_cached:
    ...
    framefinery_bytes = framefinery_output.stat().st_size
    reference_bytes = reference_output.stat().st_size
    ratio = framefinery_bytes / reference_bytes if reference_bytes else float("inf")
    return ComparisonResult(
        vector_name=vector.filename,
        framefinery_output=framefinery_output,
        reference_output=reference_output,
        framefinery_bytes=framefinery_bytes,
        reference_bytes=reference_bytes,
        ratio=ratio,
        framefinery_seconds=framefinery_seconds,
        framefinery_fps=framefinery_fps,
...
def case_paths(stem: str, args: argparse.Namespace) -> tuple[Path, Path, Path]:
    ...
    framefinery_extension = codec_extension(args.codec)
    reference_extension = reference_extension_for_args(args)
    framefinery_output = output_dir / f"{stem}_framefinery.{framefinery_extension}"
    reference_output = output_dir / f"{stem}_reference.{reference_extension}"
    log = log_dir / f"{args.set}_{stem}.log"
    return framefinery_output, reference_output, log
...
def rav1e_reference_encode_command(
    ...
    if effective_framefinery_lossless(vector, args):
        raise SystemExit(
...
def vvc_reference_encode_command(
    ...
    if bit_depth is not None:
        ...
        if not vvc_bit_depth_is_supported(bit_depth):
            raise SystemExit(
                f"unsupported VVC reference encode pixel format for native FrameFinery comparison: {vector.fmt}"
            )
    else:
        bit_depth = generate_test_vectors.yuv422_bit_depth(vector.fmt)
        if bit_depth is not None:
